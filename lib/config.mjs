...
import { alternativeValidKeyModifiers, validKeyModifiers, validKeys } from './keys.mjs'

export function parseConfig(config) {
  const { preferredinput: preferredInput = null, actions: actions = [] } = lowercaseObjectPropsDeep(config)
  const matchersByType = new Map()
  for (const { key: keyDef, midi } of actions) {
    const key = parseActions(keyDef)
  ...
  return { preferredInput: preferredInput, matcherFunctionsByType }
}
function parseActions(keyDef) {
...
function parseKeyStroke(keyDef) {
  ...
  const modifiers = parts
    ...
    .map((key) => alternativeValidKeyModifiers[key] ?? key)
  const invalidModifiers = modifiers.filter((m) => !validKeyModifiers.has(m))
  if (invalidModifiers.length > 0) {
    throw new Error(`Invalid key stroke: "${keyDef}". Invalid modifiers: "${invalidModifiers.join('", "')}"`)
  }
...
function lowercaseObjectPropsDeep(o) {
  if (o == null || typeof o !== 'object') {
    return o
  }
  if (Array.isArray(o)) {
    return o.map(lowercaseObjectPropsDeep)
  }
  const result = {}
  for (const [k, v] of Object.entries(o)) {
    result[k.toLowerCase()] = lowercaseObjectPropsDeep(v)
  }
  return result
}
const allChannels = 0
